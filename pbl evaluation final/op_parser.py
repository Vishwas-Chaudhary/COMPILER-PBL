...
def generate_assembly_from_tac(tac_lines, result_var):
    """
    Convert TAC into simple pseudo-assembly.
    Uses two registers: R1 (accumulator) and R2 (right operand).
    Variables are LOAD-ed from memory and results are STORE-d back.
    """
    asm  = []
    mmap = {}   # var -> memory label  e.g. "mem[a]"
    ...
    if not tac_lines:
        e(f'LOAD   R1, {result_var}')
        e(f'RETURN R1')
        return asm, mmap

    for tac in tac_lines:
        e(f'; {tac}')
        try:
            lhs, rhs = tac.split('=', 1)
            lhs   = lhs.strip()
            ...
            if len(parts) != 3:
                continue
            left_op, op, right_op = parts
            mem(lhs); mem(left_op); mem(right_op)

            e(f'LOAD   R1, {mem(left_op)}')
            e(f'LOAD   R2, {mem(right_op)}')
            e(f'{OP.get(op, op):<6} R1, R2')
            e(f'STORE  {mem(lhs)}, R1')
            e('')
        except Exception:
            pass

    e(f'LOAD   R1, {mem(result_var)}')
    e(f'RETURN R1')
